...
train = pandas.read_csv(path_train)
embedding_rates = train.image.apply(
    lambda s: float(s.split('-')[0]) if '.jpg' not in s.split('-')[0] else 0
)
train['embedding_rate'] = embedding_rates

lc = plt.scatter(
    train.embedding_rate[train.label=='cover'],
    # train.ac_01_mean[train.label=='cover'],
    train.aca_01[train.label=='cover'],
    # y=train.ac_01_stdev[train.label=='cover'],
    marker='o', color='b', alpha=0.33
)
ls = plt.scatter(
    train.embedding_rate[(train.label=='stego')&(train.embedding_rate<=0.05)],
    # train.ac_01_mean[train.label=='stego'],
    # train.ac_01_stdev[train.label=='stego'],
    train.aca_01[(train.label=='stego')&(train.embedding_rate<=0.05)],
    marker='o', color='r', alpha=0.33
)
plt.ylim([0, 1e5])
